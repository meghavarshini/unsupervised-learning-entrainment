...
def make_argument_parser():
	...
	parser.add_argument("-transcript_dir", help="location of transcripts")
	parser.add_argument("-audio_dir", help="location of audio files")
	parser.add_argument("-meta_file", help="location of metadata file")
	parser.add_argument("-sph2pipe", help="location of sph2pipe software")
	return parser
	...
	for dir in listdir(audio_dir_root):
		if "fisher_eng_tr" in dir:
			...
			for subsubdir in subdir.glob("*/"):
				...
				for audio in Path(subsubdir).glob("*/"):
					print("audio file found..." + str(audio))
					sess_id = str(audio).split('_')[-1]
					#ToDo: fix the line below to work correctly
					wavscpf.write(str(audio) + ' '+ sph2pipe +' -f wav -p -c 1 ' + str(audio) + ' |\n')
