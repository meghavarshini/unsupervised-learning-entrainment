...
"""
Writing the shuffled list of feature files to a text file.
This way, if you run into any issues while generating h5 files,
    ...
    if sess_files == 1:
        ...
        with open(sess_List, "r") as f:
            ...
            if len(temp) == len(sorted(glob(feats_dir + "/*.csv"))):
                ...
                raise ValueError(
                    "sessList.txt is not an accurate list of files"
                )
    else:
...
def create_test(sessTest):
    ...
    for sess_file in sessTest:
        ...
        if np.mod(N, 2) == 0:
            ...
            spk_label = np.tile(
                [spk_base, spk_base + 1], [1, math.floor(N / 2)]
            )
        else:
            print("is not 0")
